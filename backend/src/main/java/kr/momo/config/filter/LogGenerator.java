...
public class LogGenerator {
    ...
    public void logRequest(String traceId, HttpServletRequest request) {
        ...
        String token = getCookieValue(cookies).orElse("all");

        if (token.equals("all")) {
            log.info("REQUEST [{}][{} {}][{}]", traceId, httpMethod, requestURI, token);
            return;
        }
        long userId = jwtManager.extract(token);

        log.info("REQUEST [{}] [USERID:{}] [{} {}]", traceId, userId, httpMethod, requestURI);
    }
